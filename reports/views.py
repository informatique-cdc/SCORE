...
def report_list(request):
    ...
    analyses = (
        AnalysisJob.objects.filter(project=request.project)
...
def export_duplicates_csv(request, job_pk):
    ...
    pairs = DuplicatePair.objects.filter(group__analysis_job=job).select_related(
        "doc_a", "doc_b", "group"
    )
    ...
    for pair in pairs:
        writer.writerow(
            [
                ...
                pair.doc_a.title,
                pair.doc_a.source_url,
                pair.doc_b.title,
                pair.doc_b.source_url,
...
def export_contradictions_csv(request, job_pk):
    ...
    job = get_object_or_404(AnalysisJob, pk=job_pk, project=request.project)
    contradictions = ContradictionPair.objects.filter(analysis_job=job).select_related(
    ...
    for c in contradictions:
        writer.writerow(
            [
                ...
                f"{c.confidence:.3f}",
                c.claim_a.as_text,
                c.claim_a.document.title,
                c.claim_b.as_text,
                c.claim_b.document.title,
                c.evidence[:500],
                c.resolution,
            ]
        )
...
def export_report_pdf(request, job_pk):
    ...
    response["Content-Disposition"] = f'attachment; filename="score_report_{str(job.id)[:8]}.pdf"'
    return response
